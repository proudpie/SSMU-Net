...
import sys
sys.path.append(...)
from preprocessing.preprocessing_image6 import *
...
gen = torch.load('gen_6.pkl')
gen = gen.to(DEVICE)
TP = torch.load('TP_6.pkl')
TP = TP.to(DEVICE)
MU = torch.load('MU_6.pkl')
MU = MU.to(DEVICE)
FS = torch.load('FS_6.pkl')
FS = FS.to(DEVICE)
color = [[0, 0, 0], [0, 255, 255], [0, 0, 255], [237, 145, 33], [0, 255, 0], [160, 32, 240],
        [221, 160, 221], [240, 230, 140], [255, 0, 0], [255, 255, 0], [0, 255, 127], [255, 0, 255]]
...
def clour_model(dataloader):
    with torch.no_grad():
        ...
        cv2.imwrite("Hohhot.png", out_clour)
